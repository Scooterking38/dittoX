...
class Ditto:
    ...
    def customrule(self, name, text):
        self.customrules[name] = eval(text) if isinstance(text, str) else text
    ...
    def phrase(self, template, failrule=None):
        ...
        def parse_placeholder(match):
            ...
            type_name, length_part, custom_part = map(str.strip, content[:3])
            ...
            if type_name == self.ANY:
                ...
                if expected_type is None:
                    self.error(0, 1, [type_name], failrule)
            ...
            if length_part == self.ANY:
                ...
                self.error(1, 1, [length_part], failrule)
        ...
        def call(*args):
            if len(args) != len(rules):
                self.error(2, 1, [len(rules), len(args)], failrule)
            ...
            for value, (expected_type, length_rule, custom_part) in zip(args, rules):
                ...
                if custom_part != self.ANY:
                    ...
                    if func(value) is not True:
                        self.error(2, 0, [value, custom_part], failrule)
